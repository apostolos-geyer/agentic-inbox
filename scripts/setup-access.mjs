...
const SKIP_SECRETS =
  process.env.SKIP_SECRETS === "1" || process.env.SKIP_SECRETS === "true";
...
function appMatchesHost(app, hostname) {
  ...
  if (Array.isArray(app.self_hosted_domains))
    return app.self_hosted_domains.includes(hostname);
  return false;
}
...
async function ensureApp(hostname, policyId) {
  ...
  if (existing) {
    ...
    const attached = (existing.policies || []).some(
      (p) => p === policyId || p?.id === policyId,
    );
    if (!attached && !DRY_RUN) {
      const policyIds = [
